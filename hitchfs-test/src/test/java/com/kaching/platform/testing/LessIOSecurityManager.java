...
public class LessIOSecurityManager extends SecurityManager {
  ...
  public void checkExit(int status) {
	 info("%s: exit(%d)", currentTest(getClassContext()), status);
  }

  private void info(String string, Object ... objs) {
//	  System.out.println(String.format(string, objs));
  }

  @Override
  public void checkLink(String lib) {
	  info("%s: System.loadLibrary(\"%s\")", currentTest(getClassContext()), lib);
  }

  @Override
  public void checkAwtEventQueueAccess() {
   info("%s: AwtEventQueue Access", currentTest(getClassContext()));
  }

  @Override
  public void checkPrintJobAccess() {
    info("%s: PrintJob Access", currentTest(getClassContext()));
  }

  @Override
  public void checkSystemClipboardAccess() {
    info("%s: SystemClipboard Access", currentTest(getClassContext()));
  }

  @Override
  public boolean checkTopLevelWindow(Object window) {
    info("%s: checkTopLevelWindow aka AWTPermission(\"showWindowWithoutWarningBanner\")", currentTest(getClassContext()));
    return true;
  }
  ...
  private void checkClassContextPermissions(final Class<?>[] classContext, final Predicate<Class<?>> classAuthorized) throws CantDoItException {
    ...
    if (this.reporting) {
      ...
      info("%s: No %s at %s", testName, classAuthorized, testName);
      for (StackTraceElement el : currentThread().getStackTrace()) {
    	  info("%s: Stack: %s.%s():%d", testName, el.getClassName(), el.getMethodName(), el.getLineNumber());
      }
      for (Class<?> cl : classContext) {
        info("%s: Class Context: %s %s", testName, cl.getCanonicalName(), cl);
      }
